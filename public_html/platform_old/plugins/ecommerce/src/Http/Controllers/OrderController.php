...
class OrderController extends BaseController
{
    ...
    public function getReorder(Request $request, BaseHttpResponse $response)
    {
        ...
        if (is_plugin_active('payment')) {
            // Check each payment method status
            foreach (['cod', 'bank_transfer'] as $method) {
                if (get_payment_setting('status', $method) == 1) {
                    $activePaymentMethods[] = $method;
                }
            }
        }

        // If no active methods, default to bank transfer only
        if (empty($activePaymentMethods)) {
            $activePaymentMethods = ['bank_transfer'];
        }
        // dd($activePaymentMethods);
