...
try {
    if (function_exists('theme_option')) {
        $options = [
            ...
            'event_impact_desc' => "OD Sports has been the official event and digital partner for the Islamabad Marathon since 2022 helping grow it from 500 participants to 6,000+ and establishing it as Pakistan's largest marathon. We also led the multi-city execution of YourPace by inDrive across Islamabad and Karachi.",
        ...
        // Final DB forced sync for those keys
        foreach ($options as $key => $value) {
             \DB::table('settings')->where('key', 'like', '%' . $key)->update(['value' => $value]);
        }

        echo "Successfully performed full definitive update of Event Management page.\n";
    }
} catch (\Exception $e) {
