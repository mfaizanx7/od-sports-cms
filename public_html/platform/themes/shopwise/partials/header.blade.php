<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" {!! BaseHelper::siteLanguageDirection() == 'rtl' ? 'dir="rtl"' : '' !!}>
<head>
    <meta charset="utf-8">
    ...
    <title>{{ Theme::get('pageName') }} | OD Sports</title>
    ...
    <style>
        ...
        /* Services Dropdown Fix — desktop hover */
        ...
        /* Mobile: dropdown controlled by JS class, not hover */
        @media (max-width: 1024px) {
            ...
            .navbar li.dropdown.mobile-dd-open .dropdown-menu {
                display: block !important;
            }
        }

        /* Hover Bridge to prevent losing hover in the 15px gap */
    ...
    <!-- Landing Assets (Loaded after Theme::header for priority) -->
    <link rel="stylesheet" href="{{ asset('landing-assets/style.css') }}">
    <link rel="stylesheet" href="{{ asset('landing-assets/global-bridge.css') }}">
</head>

<body class="{{ theme_option('body_class') }} dark-mode" @if (BaseHelper::siteLanguageDirection() == 'rtl') dir="rtl" @endif>
    ...
    <nav class="navbar" style="z-index: 10000;">
        <div class="container">
            ...
            <ul class="nav-links">
                ...
                <li><a href="{{ route('public.custom-orders') }}">{!! theme_option('global_nav_orders', 'CUSTOM ORDERS') !!}</a></li>
                <li><a href="{{ route('public.blog') }}">{!! theme_option('global_nav_blog', 'BLOGS') !!}</a></li>
            </ul>
            <div class="nav-actions flex items-center gap-4">
                <a href="{!! theme_option('global_navbar_cta_link', route('public.index') . '#contact') !!}" class="primary-btn" style="padding: 10px 15px; font-size: 0.75rem; text-decoration: none; border-radius: 5px;">{!! theme_option('global_navbar_cta_text', 'FREE CONSULTATION') !!}</a>

            </div>
            <button class="mobile-menu-toggle" id="shopwise-hamburger" aria-label="Toggle menu">
                <span></span>
                <span></span>
                <span></span>
            </button>
    ...
    <!-- START HEADER -->
    <header class="header_wrap @if (theme_option('enable_sticky_header', 'yes') == 'yes') header_with_topbar @endif" style="margin-top: 70px;">
        <!-- Navbar -->
        <div class="relative sticky top-0 z-50">

            <!-- Category Grid Section -->
            <section class="w-full shadow-md border-b m-auto text-black" style="display: none;">
...
@php
    // Reverted sections to direct inclusion
@endphp
    <script>
